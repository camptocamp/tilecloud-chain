...
- `pathlib` must not be used, use `anyio.Path` instead.
  - **Exception**: Test files in `tilecloud_chain/tests/` may use `pathlib.Path` for synchronous operations since test utilities and setup/teardown methods often require synchronous file operations.
